...
public class SlidingWindowMaximum {
    ...
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        int[] result = new int[length - k + 1];//length - k + 1: 随着滑动窗口的移动，result 数组可以装多少个最大值
        int max = findMax(nums, 0, k - 1);//得到窗口第一次所在位置最大值
        for (int i = 0, j = k; j < length; i++, j++) {
            result[i] = max;//结果下标 = 移动窗口的开始元素下标
            if (nums[i] == max) {//nums[i] 表示将要挪掉的元素，如果是上一个窗口的 max，那么就重新找一个 max（查找范围不包含即将加入的元素）
                max = findMax(nums, i + 1, j);
            } else if (nums[j] > max) {//nums[j] 表示即将要加进来的元素，如果它比上一个窗口 max 还要大，那么直接替换 max
                max = nums[j];
            }
            //如果即将挪掉的元素比上一个窗口 max 还小，新增的元素也没有 max 大，那么直接 max值不变，直接滑倒下一个窗口位置
        }
        result[length - k] = max;//给最后一个窗口赋值
        return result;
    }
